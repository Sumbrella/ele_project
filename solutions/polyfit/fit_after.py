...
def fit_file(file_path, save_path):
    ...
    for point_id, points in enumerate(reader()):
        for point in points:
            ...
            params = fit_point(point, show=False)
...
def fit_dir(dir_path, save_path):
    files = os.listdir(dir_path)
    for file in files:
        try:
            file_path = os.path.join(dir_path, file)
            fit_file(file_path, save_path=save_path)
...
if __name__ == '__main__':
    fit_dir(os.path.join(train_dir, 'after'), save_path=os.path.join(train_dir, 'teacher'))
    fit_dir(os.path.join(test_dir, 'after'), save_path=os.path.join(test_dir, 'teacher'))
